...
class DateTimeFormatter:
    ...
    def gs1_to_ui_date(gs1_date: str) -> Optional[str]:
        ...
        if not gs1_date or len(gs1_date) != 6: return None
        try:
    ...
    def gs1_to_ui_time(gs1_time: str) -> Optional[str]:
        ...
        if not gs1_time or len(gs1_time) != 4: return None
        try:
    ...
    def ui_date_to_iso(ui_date: str) -> Optional[str]:
        ...
        Vital para MQTT/Backend que recibe los datos del PaletScanData (que están en formato UI).
        """
        if not ui_date: return None
        try:
    ...
    def ui_time_to_iso(ui_time: str) -> Optional[str]:
        ...
        if not ui_time: return None
        try:
            # Asumimos segundos 00 para la conversión
